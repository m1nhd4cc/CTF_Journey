The challenge is based on https://orlp.net/blog/subtraction-is-functionally-complete/.

To verify a part of the flag, the challenge creates an array of floating point values. Every value is either positive or negative zero. The initial sign bits depend on the bits in the input string. Negative zero is a '0'-bit, positive zero is a '1'-bit. The challenge uses floating point operations to execute operations such as xor on the sign bits. If the result matches a specific value, the input string was correct.

One way to solve this challenge is with Z3. IDA and Ghidra's decompilers create code that can be used with Z3 quite easily. However, it is important to solve every iteration of the loop separately. Otherwise, Z3 takes way too long to complete.

The following script is based on IDA's decompiler output, after turning the floats in the decompiler view into an array called v. It solves the first part of the flag. The second part can be solved with the same script, when the solve_final function is updated with the equation of check2.
